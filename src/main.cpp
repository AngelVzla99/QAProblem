...
void run_benchmark(const string problem_name = "", const string type_alg = "",
                   const int limit = -1, int maxTime = 20 * 60) {
  vector<string> problems;
  if (problem_name != "") {
...
void print_help() {
  ...
  cout << "-b2 --benchmark2: run benchmark with a specific algorithm in the "
          "second argument\n";
}
          ...
          for (auto max_iter : max_iters) {
            for (auto step : steps) {
              auto f = [&](QAP qap, QAP_solution &ans) {
                return multi_modal_solution(qap, ans, 5, pop_size, max_iter,
                                            step, false);
              };

              run_benchmark_V2(
                  f, type_alg,
                  "multi_modal_LS:75_popSize:" + to_string(pop_size) +
                      "_maxIter:" + to_string(max_iter) +
        ...
        for (auto n_stu : n_students) {
          for (auto n_tea : n_teachers) {
            for (auto n_iter : max_iterations) {
              ...
              auto f = [&](QAP qap, QAP_solution &ans) {
                return teacher_student_solution(qap, ans, n_stu, n_tea, n_iter,
                                                false);
              };

              run_benchmark_V2(f, type_alg,
                               "teacher_student_nStudents:" + to_string(n_stu) +
                                   "_nTeachers:" + to_string(n_tea) +
                                   "_maxIter:" + to_string(n_iter));
            }
          }
        }
      }
